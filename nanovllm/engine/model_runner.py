...
swap_ops = load_extension(name="swap_ops",
                          sources=[os.path.join(_csrc_dir, "swap_kernels.cu")],
                          verbose=False)
...
class ModelRunner:
    ...
    def allocate_kv_cache(self):
        ...
        # CPU swap space
        config.num_cpu_blocks = int(config.cpu_swap_space_gb * 2**30) // block_bytes if config.cpu_swap_space_gb > 0 else 0
        if config.num_cpu_blocks > 0:
            self.cpu_kv_cache = torch.empty(
                2, hf_config.num_hidden_layers, config.num_cpu_blocks, self.block_size, num_kv_heads, head_dim,
    ...
    def swap_blocks(self, src, dst, mappings):
        if not mappings:
            return
        block_mapping = torch.tensor(mappings, dtype=torch.int64, device="cuda")
